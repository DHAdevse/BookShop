...
public class VNPayService {
    ...
    public String createOrder(HttpServletRequest request, double amount, String orderInfor, String urlReturn){
        ...
        //TotalPay
        int temp = (int)amount;
        vnp_Params.put("vnp_Amount", String.valueOf(temp*100));
        vnp_Params.put("vnp_CurrCode", "VND");

        vnp_Params.put("vnp_TxnRef", vnp_TxnRef);
        orderInfor = invoice.getInvoiceId();
        ...
        if (signValue.equals(vnp_SecureHash)) {
            if ("00".equals(request.getParameter("vnp_TransactionStatus"))) {
                ...
                for(LineItem item: cart.getLineItemList())
                {
                    ...
                    int quantity = book.getStock().getQuantity() - item.getQuantity();
                    book.getStock().setQuantity(quantity);
                    bookService.save(book);
                }
                cart.getLineItemList().clear();
